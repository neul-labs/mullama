...
use crate::{context::Context, token::TokenId, error::MullamaError, sys, batch::Batch};
...
impl EmbeddingGenerator {
    ...
    pub fn embed_tokens(&mut self, tokens: &[TokenId]) -> Result<Vec<f32>, MullamaError> {
        if tokens.is_empty() {
            return Err(MullamaError::InvalidInput("Cannot embed empty token sequence".to_string()));
        }
        ...
        let embedding = match self.config.pooling {
            PoolingStrategy::Native => {
                ...
                self.context.get_embeddings_seq(0)
                    .map(|e| e.to_vec())
                    .ok_or_else(|| MullamaError::EmbeddingError(
            ...
            PoolingStrategy::Last => {
                ...
                self.context.get_embeddings_ith(last_idx)
                    .map(|e| e.to_vec())
                    .ok_or_else(|| MullamaError::EmbeddingError(
            ...
            PoolingStrategy::First => {
                ...
                self.context.get_embeddings_ith(0)
                    .map(|e| e.to_vec())
                    .ok_or_else(|| MullamaError::EmbeddingError(
...
impl EmbeddingUtil {
    ...
    pub fn generate_embeddings(context: &mut Context, tokens: &[TokenId]) -> Result<Embeddings, MullamaError> {
        if tokens.is_empty() {
            return Err(MullamaError::InvalidInput("Cannot embed empty token sequence".to_string()));
        }
        ...
        for i in 0..tokens.len() {
            if let Some(emb) = context.get_embeddings_ith(i as i32) {
                ...
                return Err(MullamaError::EmbeddingError(
                    format!("Failed to get embedding for token at index {}", i)
                ));
            }
        }
